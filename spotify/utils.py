...
def refresh_spotify_token(session_key):
	...
	response = post('https://accounts.spotify.com/api/token', date={
...
def execute_spotify_api_calls(session_key, endpoint, post_=False, put_=False):
	tokens = get_user_tokens(session_key)
...
def pause_song(session_key):
	return execute_spotify_api_calls(session_key, BASE_URL + "player/play", put_=True)

def play_song(session_key):
	return execute_spotify_api_calls(session_key, BASE_URL + "player/pause", put_=True)