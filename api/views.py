...
class JoinRoom(APIView):
	...
	def post(self, request, format=None):
		...
		if code != None:
			...
			return Response({'Bad Request' : 'Invalid Room Code Data'}, status=status.HTTP_400_BAD_REQUEST)
...
class LeaveRoom(APIView):
	def post(self, request, format=None):
		...
		return Response({'message' : 'You are currently not in a room'}, status=status.HTTP_304_NOT_MODIFIED)
